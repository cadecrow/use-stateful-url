...
import { hashUtils } from "../src/useStatefulUrl";
...
describe("Hash Utils - DOM Integration", () => {
	...
	test("should get state from hash when present", () => {
		window.location.hash =
			"#external=value&__UHS-count=5&name=test-UHS__&other=param";
	...
	test("should work with custom delimiters", () => {
		...
		window.location.hash =
			"#external=value&<<<count=5&name=test>>>&other=param";
	...
	test("should update external hash safely", () => {
		...
		hashUtils.setExternalHash("newexternal=value");
	...
	test("should trigger hashchange event on external update", (done) => {
		...
		hashUtils.setExternalHash("external=updated", undefined, false);
	});
